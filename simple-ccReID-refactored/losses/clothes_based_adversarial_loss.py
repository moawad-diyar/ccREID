...
class ClothesBasedAdversarialLossWithMemoryBank(nn.Module):
    ...
    def forward(self, inputs, targets, positive_mask):
        ...
        """
        # Update memory with current batch (no distributed gathering needed for single GPU)
        ...
        negtive_mask = 1 - positive_mask
        mask_identity = torch.zeros(positive_mask.size()).scatter_(1, targets.unsqueeze(1).data.cpu(), 1).to(inputs.device)
