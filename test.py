...
@torch.no_grad()
def extract_img_feature(model, dataloader, device):
...
@torch.no_grad()
def extract_vid_feature(model, dataloader, vid2clip_index, device):
    logger = logging.getLogger('reid.test')
    # In build_dataloader, each original test video is split into a series of equilong clips.
    # During test, we first extract features for all clips
...
def test(config, model, queryloader, galleryloader, dataset, device):
    logger = logging.getLogger('reid.test')
    since = time.time()
    model.eval()
    ...
    if config.DATA.DATASET in VID_DATASET:
        qf, q_pids, q_camids, q_clothes_ids = extract_vid_feature(model, queryloader, 
                                                                  dataset.query_vid2clip_index, device)
        gf, g_pids, g_camids, g_clothes_ids = extract_vid_feature(model, galleryloader, 
                                                                  dataset.gallery_vid2clip_index, device)
    else:
        qf, q_pids, q_camids, q_clothes_ids = extract_img_feature(model, queryloader, device)
        gf, g_pids, g_camids, g_clothes_ids = extract_img_feature(model, galleryloader, device)
...
def test_prcc(model, queryloader_same, queryloader_diff, galleryloader, dataset, device):
    logger = logging.getLogger('reid.test')
    since = time.time()
    model.eval()
    
    # Extract features for query set
    qsf, qs_pids, qs_camids, qs_clothes_ids = extract_img_feature(model, queryloader_same, device)
    qdf, qd_pids, qd_camids, qd_clothes_ids = extract_img_feature(model, queryloader_diff, device)
